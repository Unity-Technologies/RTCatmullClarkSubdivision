...
RWStructuredBuffer<uint> u_CbtBuffer;
#else
StructuredBuffer<uint> u_CbtBuffer;
#endif
...
void cbt__SetBitValue(uint bufferID, uint bitID, uint bitValue)
{
    ...
    InterlockedAnd(u_CbtBuffer[bufferID], bitMask);
    InterlockedOr(u_CbtBuffer[bufferID], bitValue << bitID);
}
 ...
 */
void
cbt__BitFieldInsert(uint bufferID, uint bitOffset, uint bitCount, uint bitData)
{
    uint bitMask = ~(~(0xFFFFFFFFu << bitCount) << bitOffset);

    InterlockedAnd(u_CbtBuffer[bufferID], bitMask);
    InterlockedOr(u_CbtBuffer[bufferID], bitData << bitOffset);
}
...
cbt_Node cbt_ParentNode(in const cbt_Node node)
{
    if (cbt_IsNullNode(node))
        ...
        return cbt_ParentNode_Fast(node);
}
...
cbt_Node cbt__CeilNode(in const cbt_Node node)
{
    if (cbt_IsNullNode(node))
        ...
        return cbt__CeilNode_Fast(node);
}
...
cbt_Node cbt_SiblingNode(in const cbt_Node node)
{
    if (cbt_IsNullNode(node))
        ...
        return cbt_SiblingNode_Fast(node);
}
...
cbt_Node cbt_RightSiblingNode(in const cbt_Node node)
{
    if (cbt_IsNullNode(node))
        ...
        return cbt_RightSiblingNode_Fast(node);
}
...
cbt_Node cbt_LeftSiblingNode(in const cbt_Node node)
{
    if (cbt_IsNullNode(node))
        ...
        return cbt_LeftSiblingNode_Fast(node);
}
...
cbt_Node cbt_RightChildNode(in const cbt_Node node)
{
    if (cbt_IsNullNode(node))
        ...
        return cbt_RightChildNode_Fast(node);
}
...
cbt_Node cbt_LeftChildNode(in const cbt_Node node)
{
    if (cbt_IsNullNode(node))
        ...
        return cbt_LeftChildNode_Fast(node);
}
 ...
 *
 */
int cbt__NodeBitSize(int depth)
{
...
{
    cbt__HeapArgs args = cbt__CreateHeapArgs(node, bitCount);
    uint lsb = cbt__BitFieldExtract(u_CbtBuffer[args.heapIndexLSB],
                                    args.bitOffsetLSB,
                                    args.bitCountLSB);
    uint msb = cbt__BitFieldExtract(u_CbtBuffer[args.heapIndexMSB],
                                    0u,
                                    args.bitCountMSB);
...
uint cbt__HeapRead_BitField(in const cbt_Node node)
{
    ...
    return cbt__GetBitValue(u_CbtBuffer[bitID >> 5u], bitID & 31u);
}
...
bool cbt_IsLeafNode(in const cbt_Node node)
{
    if (cbt_IsCeilNode(node)) {
        ...
        const cbt_Node rightChild = cbt_RightChildNode_Fast(node);
        return (cbt__HeapRead_BitField(rightChild) == 0u);
    }
}
...
int cbt_MaxDepth()
{
    return firstbitlow(u_CbtBuffer[0]);
}
 ...
 *
 */
void cbt__Decode(inout cbt_Node node, uint heap, uint nodeID)
{
    uint bitMask = 0xFFFFu;
    uint bitShift = 16u;

	int safeGuard = 0;
    while (countbits(heap) > 1u && safeGuard < 100)
	{
        const uint cmp = countbits(heap & bitMask);
        const uint b = nodeID < cmp ? 0u : 1u;

        node.id = (node.id * 2u) + b;
        node.depth += 1;
        nodeID -= cmp * b;
        heap = ((heap >> (b * bitShift)) & bitMask);
        bitShift >>= 1u;
        bitMask >>= bitShift;
...
cbt_Node cbt_DecodeNode(uint nodeID)
{
    const int maxDepth = cbt_MaxDepth();
    const int limDepth = maxDepth - 5;
    cbt_Node node = cbt_CreateNode(1u, 0);

	int safeGuard = 0;
    while (cbt_HeapRead(node) > 1u && node.depth < limDepth && safeGuard < 100)
	{
    ...
    if (node.depth == limDepth)
	{
        const uint offset = 1u << (maxDepth - 4);
        const uint heapID = offset + (node.id ^ (1u << limDepth));
        const uint heap = u_CbtBuffer[heapID];

        cbt__Decode(node, heap, nodeID);
    }
...
uint cbt_EncodeNode(in const cbt_Node node)
{
	...
	int safeGuard = 0;
    while (nodeIterator.id > 1u && safeGuard < 100)
	{
